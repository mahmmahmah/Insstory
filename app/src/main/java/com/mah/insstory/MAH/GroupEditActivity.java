...
public class GroupEditActivity extends AppCompatActivity {
	...
	private StorageReference gadb = _firebase_storage.getReference("groups/avatars");
	private OnCompleteListener<Uri> _gadb_upload_success_listener;
	private OnSuccessListener<FileDownloadTask.TaskSnapshot> _gadb_download_success_listener;
	private OnSuccessListener _gadb_delete_success_listener;
	private OnProgressListener _gadb_upload_progress_listener;
	private OnProgressListener _gadb_download_progress_listener;
	private OnFailureListener _gadb_failure_listener;
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		_gdb_child_listener = new ChildEventListener() {
			...
			public void onChildAdded(DataSnapshot _param1, String _param2) {
				...
				if (_childKey.equals(getIntent().getStringExtra("group_key"))) {
					AvatarPath = _childValue.get("group_avatar").toString();
		...
		_gadb_upload_progress_listener = new OnProgressListener<UploadTask.TaskSnapshot>() {
			@Override
		...
		_gadb_download_progress_listener = new OnProgressListener<FileDownloadTask.TaskSnapshot>() {
			@Override
		...
		_gadb_upload_success_listener = new OnCompleteListener<Uri>() {
			@Override
			public void onComplete(Task<Uri> _param1) {
				final String _downloadUrl = _param1.getResult().toString();
				AvatarPath = _downloadUrl;
			}
		};
		
		_gadb_download_success_listener = new OnSuccessListener<FileDownloadTask.TaskSnapshot>() {
			@Override
		...
		_gadb_delete_success_listener = new OnSuccessListener() {
			@Override
		...
		_gadb_failure_listener = new OnFailureListener() {
			@Override
	...
	protected void onActivityResult(int _requestCode, int _resultCode, Intent _data) {
		...
		switch (_requestCode) {
			...
			if (_resultCode == Activity.RESULT_OK) {
				...
				AvatarName = getIntent().getStringExtra("group_key").concat(".png");
				group_avatar.setImageBitmap(FileUtil.decodeSampleBitmapFromPath(AvatarPath, 1024, 1024));
				gadb.child(AvatarName).putFile(Uri.fromFile(new File(AvatarPath))).addOnFailureListener(_gadb_failure_listener).addOnProgressListener(_gadb_upload_progress_listener).continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
					@Override
					public Task<Uri> then(Task<UploadTask.TaskSnapshot> task) throws Exception {
						return gadb.child(AvatarName).getDownloadUrl();
					}}).addOnCompleteListener(_gadb_upload_success_listener);
			}
			else {
		...
		if (save1.getString("theme", "").equals("light")) {
			...
			if (save1.getString("theme", "").equals("dark")) {
				if (Build.VERSION.SDK_INT > Build.VERSION_CODES.KITKAT) {
					Window w =GroupEditActivity.this.getWindow();
	...
	public void _EnLang() {
		...
		ChangesSavedString = "Changes Saved!";
	}
