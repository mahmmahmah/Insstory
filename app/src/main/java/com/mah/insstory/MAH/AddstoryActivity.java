...
public class AddstoryActivity extends AppCompatActivity {
	...
	private StorageReference ssb = _firebase_storage.getReference("social/stories");
	private OnCompleteListener<Uri> _ssb_upload_success_listener;
	private OnSuccessListener<FileDownloadTask.TaskSnapshot> _ssb_download_success_listener;
	private OnSuccessListener _ssb_delete_success_listener;
	private OnProgressListener _ssb_upload_progress_listener;
	private OnProgressListener _ssb_download_progress_listener;
	private OnFailureListener _ssb_failure_listener;
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		save.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (avatarPath.equals("")) {
					SketchwareUtil.showMessage(getApplicationContext(), select_image);
				}
				else {
					ssb.child(avatarName).putFile(Uri.fromFile(new File(avatarPath))).addOnFailureListener(_ssb_failure_listener).addOnProgressListener(_ssb_upload_progress_listener).continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
						@Override
						public Task<Uri> then(Task<UploadTask.TaskSnapshot> task) throws Exception {
							return ssb.child(avatarName).getDownloadUrl();
						}}).addOnCompleteListener(_ssb_upload_success_listener);
				}
			}
		});
		...
		_ssb_upload_progress_listener = new OnProgressListener<UploadTask.TaskSnapshot>() {
			@Override
		...
		_ssb_download_progress_listener = new OnProgressListener<FileDownloadTask.TaskSnapshot>() {
			@Override
		...
		_ssb_upload_success_listener = new OnCompleteListener<Uri>() {
			@Override
			public void onComplete(Task<Uri> _param1) {
				final String _downloadUrl = _param1.getResult().toString();
				cc = Calendar.getInstance();
				PushKey = sdb.push().getKey();
				map = new HashMap<>();
				map.put("push_date", String.valueOf((long)(cc.getTimeInMillis())));
				map.put("story", "true");
				map.put("image", _downloadUrl);
				map.put("push_key", PushKey);
				map.put("uid_key", FirebaseAuth.getInstance().getCurrentUser().getUid());
				map.put("uid", FirebaseAuth.getInstance().getCurrentUser().getUid());
				sdb.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).updateChildren(map);
				map.clear();
				map = new HashMap<>();
				map.put("story", "true");
		...
		_ssb_download_success_listener = new OnSuccessListener<FileDownloadTask.TaskSnapshot>() {
			@Override
		...
		_ssb_delete_success_listener = new OnSuccessListener() {
			@Override
		...
		_ssb_failure_listener = new OnFailureListener() {
			@Override
	...
	protected void onActivityResult(int _requestCode, int _resultCode, Intent _data) {
		...
		switch (_requestCode) {
			...
			if (_resultCode == Activity.RESULT_OK) {
				...
				avatarName = sdb.push().getKey().concat(".png");
				image.setImageBitmap(FileUtil.decodeSampleBitmapFromPath(avatarPath, 1024, 1024));
	...
	public void _TrLang() {
		title.setText("Hikaye Ekle");
		
		
		success = "Hikaye Paylaşıldı!";
	...
	public void _EnLang() {
		...
		select_image = "Please Select Picture";
	}
	
	
	public void _InLang() {
		title.setText("Tambahkan Cerita");
		
		
		success = "Cerita Dibagikan!";
		select_image = "Silahkan Pilih Gambar";
	}
